...
public abstract class AbstractPlayValidator {
   private static final Logger logger = LoggerFactory.getLogger(AbstractPlayValidator.class);
   private static final String PACKAGE_NAME = AbstractPlayValidator.class.getPackage().getName();
    ...
    * @param errors
    */
   protected void checkNonUpdateableArgs(PlayResource target, Errors errors) {
      final String proc = PACKAGE_NAME + ".checkNonUpdateableArgs.";
      final String playId = target.getPlayId();
      final String playType = target.getPlayType();
      final PlayKey playKey = target.getPlayKey();
      final String eventId = target.getGameId();
   ...
   /**
    * Validates that the team is a valid team. If the given team is participating in the event, then
    * processing continues.
    *
    * @param target
    * @param errors
    */
   protected void checkTeam(PlayResource target, Errors errors) {
   ...
   /**
    * Validates that the period is valid. If the period is an integer >0 then processing continues.
    *
    * @param target
    * @param errors
    */
   protected void checkPeriod(PlayResource target, Errors errors) {
