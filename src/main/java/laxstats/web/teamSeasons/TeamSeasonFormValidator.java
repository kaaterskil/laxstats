...
public class TeamSeasonFormValidator implements Validator {
	private static final Logger logger = LoggerFactory
			.getLogger(TeamSeasonFormValidator.class);
	private static final String PACKAGE_NAME = TeamSeasonFormValidator.class
			.getPackage().getName();
	...
	private void checkMandatoryArgs(TeamSeasonForm form, Errors errors) {
		...
		ValidationUtils.rejectIfEmpty(errors, "status",
				"teamSeason.status.required");

		logger.debug("Leaving: " + proc + "40");
	}

	/**
	 * Validates that non-updateable values have not changed. Specifically, this
	 * method validates that the team and season have not changed if games are
	...
	private void checkNonUpdateableArgs(TeamSeasonForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			if (team.getEvents().size() > 0) {
				...
				if (seasonId == null
						|| !team.getSeason().getId().equals(seasonId)) {
					errors.rejectValue("season",
							"teamSeason.season.notUpdateable");
				}
			}
		}
	...
	private void checkDuplicate(TeamSeasonForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			if (!team.getTeam().getId().equals(teamId)
				...
				found = teamRepository.updateSeason(teamId, seasonId,
						teamSeasonId);
				if (found > 0) {
	...
	/**
	 * Validates that the team season start date, if set, is within the
	 * specified season date range.
	 *
	 * @param form
	 * @param errors
	 */
	private void checkStartDate(TeamSeasonForm form, Errors errors) {
		...
		if (startsOn != null) {
			...
			if (isUpdating) {
				...
				final TeamSeasonEntry team = teamRepository
						.findOne(teamSeasonId);
				if (!team.getStartsOn().equals(startsOn)) {
					logger.debug(proc + "40");

					if (startsOn.isBefore(season.getStartsOn())) {
						errors.rejectValue("startsOn",
								"teamSeason.startsOn.tooEarly");
					} else if (startsOn.isAfter(Common.nvl(season.getEndsOn(),
							eot))) {
						errors.rejectValue("startsOn",
								"teamSeason.startsOn.tooLate");
					}
				}
			} else {
				logger.debug(proc + "50");
	...
	/**
	 * Validates that the team season end date, if set, is within the specified
	 * season date range.
	 *
	 * @param form
	 * @param errors
	...
	private void checkEndDate(TeamSeasonForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			final TeamSeasonEntry team = teamRepository.findOne(teamSeasonId);
			if (!Common.nvl(team.getEndsOn(), eot).equals(
				...
				} else if (Common.nvl(endsOn, eot).isAfter(
						Common.nvl(season.getEndsOn(), eot))) {
	...
	/**
	 * Validates that the team season start date, if set, is not earlier than
	 * the end date.
	 *
	 * @param form
	 * @param errors
		...
		if (startsOn != null) {
			...
			if (isUpdating) {
				...
				final TeamSeasonEntry team = teamRepository
						.findOne(teamSeasonId);
				if (!team.getStartsOn().equals(startsOn)
						|| !Common.nvl(team.getEndsOn(), eot).equals(
								Common.nvl(endsOn, eot))) {
					logger.debug(proc + "40");

					if (startsOn.isAfter(Common.nvl(endsOn, eot))) {
						errors.rejectValue("startsOn",
								"teamSeason.invalidDateRange");
					}
				}
			} else {
				logger.debug(proc + "50");
