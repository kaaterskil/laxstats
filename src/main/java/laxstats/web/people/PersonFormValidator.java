...
public class PersonFormValidator implements Validator {
	private static final Logger logger = LoggerFactory
			.getLogger(PersonFormValidator.class);
	private static final String PACKAGE_NAME = PersonFormValidator.class
			.getPackage().getName();
	...
	/**
	 * Validates that a birth date is valid. If the played seasons collection is
	 * not empty and the birth date is <= the earliest season, then processing
	 * continues.
	 *
	 * @param form
	 * @param errors
		...
		if (birthDate != null) {
			...
			if (isUpdating) {
				...
				if (!person.getPlayedSeasons().isEmpty()
						&& !Common.nvl(person.getBirthdate(), eot).equals(
								birthDate)) {
					logger.debug(proc + "40");
	...
	/**
	 * Validates that a gender is valid. If the played seasons collection is not
	 * empty and the person is an athlete and their gender equals the team
	 * gender, then processing continues.
	 *
	 * @param form
	 * @param errors
	 */
	private void checkGender(PersonForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			if (!person.getPlayedSeasons().isEmpty()) {
				...
				final TeamGender genderType = personRepository.getTeamGender(
						personId, Role.ATHLETE);
				if (genderType != null) {
					logger.debug(proc + "50");

					if (genderType.equals(TeamGender.BOYS)
							|| genderType.equals(TeamGender.MEN)) {
	...
	/**
	 * Validates that the date the parent release was sent. If the parent
	 * release sent date is before the received on date , or if the received on
	 * date is null, the processing continues.
	 *
	 * @param form
	 * @param errors
	 */
	private void checkParentReleaseSentOn(PersonForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			final PersonEntry person = personRepository.findOne(personId);
			if (!Common.nvl(person.getParentReleaseSentOn(), eot).equals(
					Common.nvl(sentOn, eot))) {
				logger.debug(proc + "40");

				if (Common.nvl(sentOn, eot)
						.isAfter(Common.nvl(receivedOn, eot))) {
					errors.rejectValue("parentReleaseSentOn",
							"person.parentReleaseSentOn.invalid");
				}
			}
		} else {
			logger.debug(proc + "50");
	...
	/**
	 * Validates the date the parent release was received. If the parent release
	 * received date is after the sent date, the processing continues.
	 *
	 * @param form
	 * @param errors
	 */
	private void checkParentReleaseReceivedOn(PersonForm form, Errors errors) {
		...
		if (receivedOn != null) {
			...
			if (sentOn == null) {
				errors.rejectValue("parentReleaseSentOn",
						"person.parentReleaseSentOn.required");
			}
			...
			if (isUpdating) {
				...
				final PersonEntry person = personRepository.findOne(personId);
				if (Common.nvl(person.getParentReleaseReceivedOn(), eot)
