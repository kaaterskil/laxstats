...
public class ViolationFormValidator implements Validator {
	private static final Logger logger = LoggerFactory
			.getLogger(ViolationFormValidator.class);
	private static final String PACKAGE_NAME = ViolationFormValidator.class
			.getPackage().getName();
	...
	private void checkName(ViolationForm form, Errors errors) {
		...
		if (isUpdating) {
			final ViolationEntry violation = violationRepository
					.findOne(violationId);
			logger.debug(proc + "30");

			if ((name != null && violation.getName() == null)
					|| !violation.getName().equals(name)) {
	...
	/**
	 * Returns <code>true</code> if he record with the given primary key is
