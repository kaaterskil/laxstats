...
public class EventFormValidator implements Validator {
	private static final Logger logger = LoggerFactory
			.getLogger(EventFormValidator.class);
	private static final String PACKAGE_NAME = EventFormValidator.class
			.getPackage().getName();
	...
	private void checkDuplicate(EventForm form, Errors errors) {
		...
		final String startsAt = form.getStartsAt().toString(
				"yyyy-MM-dd HH:mm:ss");
		int found = 0;
		...
		if (isUpdating) {
			...
			if (!game.getTeams().isEmpty()) {
				...
				if ((Common.nvl(oldTeamOne.getId(), "").equals(Common.nvl(
						teamOneId, "")))
						|| (Common.nvl(oldTeamTwo.getId(), "").equals(Common
								.nvl(teamTwoId, "")))) {
					logger.debug(proc + "40");

					found = gameRepository.checkUpdate(startsAt, siteId,
							teamOneId, teamTwoId, gameId);
					if (found > 0) {
			...
			logger.debug(proc + "40");
			found = gameRepository.checkUnique(startsAt, siteId, teamOneId,
					teamTwoId);
			if (found > 0) {
				errors.rejectValue("startsAt", "game.duplicate");
			}
	...
	/**
	 * Validates that only a single team is classified as a home team if the
	 * site is a designated home site, and that there is no home team if the
	 * site not a home site.
	 *
	 * @param form
	 * @param errors
	 */
		...
		if (doValidation) {
			if (alignment.equals(SiteAlignment.HOME)) {
				if (!isTeamOneHome && !isTeamTwoHome) {
					errors.rejectValue("teamOneHome",
							"game.teamOneHome.required");
				}
				if (isTeamOneHome && isTeamTwoHome) {
					errors.rejectValue("teamOneHome",
							"game.teamOneHome.twoHomeTeams");
				}
			} else {
				if (isTeamOneHome) {
					errors.rejectValue("teamOneHome",
							"game.teamOneHome.invalid");
				}
				if (isTeamTwoHome) {
					errors.rejectValue("teamTwoHome",
							"game.teamTwoHome.invalid");
				}
			}
		}
		logger.debug("Leaving: " + proc + "70");
	 ...
	 * @param gameId
	 * @return
	 * @throws IllegalStateException if no records exists for the given primary
	 *             key.
	 */
