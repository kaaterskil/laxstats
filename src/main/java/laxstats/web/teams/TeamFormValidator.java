...
public class TeamFormValidator implements Validator {
	private static final Logger logger = LoggerFactory
			.getLogger(TeamFormValidator.class);
	private static final String PACKAGE_NAME = TeamFormValidator.class
			.getPackage().getName();
	...
	private void checkTeam(TeamForm form, Errors errors) {
		...
		if (isUpdating) {
			...
			if (!team.getSponsor().equals(sponsor)
					|| !team.getName().equals(name)
					|| !team.getGender().equals(gender)
					|| !team.getLetter().equals(letter)) {
				logger.debug(proc + "40");

				found = teamRepository.updateTeam(sponsor, name, gender,
						letter, teamId);
				if (found > 0) {
	...
	/**
	 * Validates that the home site, if set and references an address, is in the
	 * same region.
	 *
	 * @param form
	 * @param errors
